...
export const ModuleSelector: React.FC<{
  ...
  renderedBoxKeys = [],
    ...
    const handleSelect = (mod: ModuleCode) =>
      selectModule(programmeId, groupType, courseBox.pathId, mod, courseBox.boxKey);
    const handleRemove = () =>
      removeModule(programmeId, groupType, courseBox.pathId, courseBox.boxKey);
    ...
    switch (courseBox.kind) {
      case 'exact': {
        ...
        const already = userModuleSelections.some(
          s => s.boxKey === courseBox.boxKey
        );

        useEffect(() => {
          if (courseBox.kind === 'exact' && !already) {
            selectModule(
              programmeId,
              groupType,
              courseBox.pathId,
        ...
        }, [already, programmeId, groupType, courseBox, selectModule]);
        return (
          <Box sx={{
      ...
      case 'altPath': {
        const childBox = alternatives[selectedAltIdx];
        if (childBox && renderedBoxKeys.includes(childBox.boxKey)) {
          return (
            <Box
              sx={{
        ...
        return (
          ...
          >
            ...
            {/* Only render the selected alt box if not already rendered at this level */}
            {childBox && (
              <BoxRenderer
                key={childBox.boxKey}
                box={childBox}
                ...
                renderedBoxKeys={[...renderedBoxKeys, courseBox.boxKey]}
              />
            )}
          </Box>
        );
