...
export class Optimizer {
    ...
    async getFilteredDropdownOptions(boxOptions: ModuleCode[]): Promise<{
        ...
        const moduleDetailsMap = new Map(moduleDetails.map(mod => [mod.moduleCode, mod]));
    ...
    private async getModuleWarnings(module: ModuleCode): Promise<string[]> {
        ...
        try {
            const prereqData = await dbService.getBatchSimplePrerequisites([module]);
            const prereqs = prereqData.find(p => p.moduleCode === module);
            if (prereqs && prereqs.requiredModules.length > 0) {
                const selectedModules = this.validator.getSelectedModules();
                const missingPrereqs = prereqs.requiredModules.filter(
                    prereq => !selectedModules.has(prereq)
                );
    ...
    getDecisionDialogContent(decision: PendingDecision): {
        ...
        const options = decision.options.map(option => ({
            ...
            utilizationBar: {
                current: option.currentUsage,
                max: option.maxCapacity,
                percentage: Math.round(option.utilizationRate * 100)
            },
            isRecommended: option.utilizationRate < 0.7
        }));
    ...
    async getModuleInfo(module: ModuleCode): Promise<{
        ...
        try {
            // Fetch module details and rules in parallel
            const [moduleDetails, prereqData, preclusionData] = await Promise.all([
                dbService.getModulesDetails([module]),
                dbService.getBatchSimplePrerequisites([module]),
            ...
            const prereqs = prereqData.find(p => p.moduleCode === module)?.requiredModules || [];
            const preclusions = preclusionData.find(p => p.moduleCode === module)?.precludedModules || [];

            return {
                moduleCode: module,
                title: moduleInfo.title,
                au: moduleInfo.moduleUnit,
